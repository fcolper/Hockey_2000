...
# url='https://api-web.nhle.com/v1/schedule/2024-06-24' ##CAMBIAR A NOW
...
playoff=pd.read_csv('Data_csv/ID_Matches_2018-2019.csv', sep=',')
...
for l in id_matches_links:
    print(l)
    url=f'https://www.nhl.com/scores/htmlreports/20182019/PL0{l}.HTM'
    #Las dos lineas siguientes es si quieres tener en el DataFrame Corsi los valores incluidos de los playoff
    ...
    if len(list_columnas) <1:
        print('list_columnas vacío', list_columnas)
        break
    
    AKA_away_team = list_columnas[6].split()[0] 
    AKA_home_team = list_columnas[7].split()[0]

    new_url=f'https://api-web.nhle.com/v1/gamecenter/20180{l}/landing'

    response=requests.get(new_url)
    data=response.json()

    AKA_away_team = data['awayTeam']['abbrev']
    AKA_home_team = data['homeTeam']['abbrev']
    ...
    playoff['gameDate'] = pd.to_datetime(playoff['gameDate'])
    fecha_actual = datetime.now()
    df_filtrado = playoff[playoff['gameDate'] <= fecha_actual]    
...
combined_df.to_csv(f'Data_csv/Test_teams_zero_2018_2019.csv', index=False)
