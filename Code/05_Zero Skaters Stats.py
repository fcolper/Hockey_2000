...
file='Data_csv/ID_Matches_2019-2020.csv'
schedule_matches=pd.read_csv(file)
# fecha_hoy = datetime.now().strftime('%Y-%m-%d')
fecha_hoy ='2019-10-03'
print('Fecha hoy:', fecha_hoy)

schedule_matches= schedule_matches[schedule_matches['gameDate'] < fecha_hoy]
schedule_matches=schedule_matches.drop_duplicates(subset='id')

large=schedule_matches['id'].count()
print('large:',large)
id_matches_Links=schedule_matches['Links']
final_list=[]
quedan=large
...
for l in id_matches_Links:
    print(l)
    print('Quedan:',quedan)
    gametype_fila=schedule_matches['Links']==l
    gametype = int(schedule_matches.loc[gametype_fila, 'gameType'].iat[0])
    quedan=quedan-1
    url=f'https://www.nhl.com/scores/htmlreports/20192020/PL0{l}.HTM' #MODIFICAR SEASON
    ...
    skaters=skaters_(f'https://api-web.nhle.com/v1/gamecenter/20190{l}/boxscore') #MODIFICAR EL AÑO
...
print(df_concat)
df_concat.to_csv("Data_csv/CorsiStats_Players_2019-2020.csv", index=False)
